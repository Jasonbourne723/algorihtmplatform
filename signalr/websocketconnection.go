...
func newWebSocketConnection(ctx context.Context, connectionID string, conn *websocket.Conn, accessToken string) *webSocketConnection {
	w := &webSocketConnection{
		conn:           conn,
		ConnectionBase: *NewConnectionBase(ctx, connectionID, accessToken),
	}
	return w
}
